...
- **Audio**: every phrase ships with a real recitation, cut from the Hisn al-Muslim recordings published by [dua.gtaf.org](https://dua.gtaf.org/) and stored as `data/audio/<id>.mp3` (id matches the `id` in `data/zikr.json`). `build.sh` bundles them; `tools/fetch_audio.py` is the pipeline that fetches and cuts them, and is the place to read for how a clip is chosen and checked. Where no clip exists, `AVSpeechSynthesizer` speaks the Arabic with the system voice ("Majed", installed on every Mac) so the phrase is still pronounced rather than chimed, falling back to the transliteration if no Arabic voice is present. Dropping a file at `Resources/Audio/<id>.mp3` (`.m4a`/`.caf`/`.wav`/`.aiff` also work) overrides the bundled clip.
